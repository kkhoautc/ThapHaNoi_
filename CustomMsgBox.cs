...
namespace ThapHaNoi
{
    public static class CustomMsgBox
    {
        ...
        public static void ShowLeaderboard(System.Data.DataTable dt, string title = "")
        {
            using (Form msgForm = new Form())
            {
                msgForm.Size = new Size(600, 450); // Bảng thành tích cần rộng hơn
        ...
        public static void ShowImage(string message, string title = "")
        {
            using (Form msgForm = new Form())
            {
                // Xác định màu viền theo Type
                Color themeColor = Color.Gold;
              

                // Form sẽ cao hơn để chứa được ảnh
                ...
                pictureBox.BackColor = Color.FromArgb(20, 20, 25); // Nền tối hơn một chút để tôn ảnh lên
                mainPanel.Controls.Add(pictureBox);

                // 3. Nội dung chữ (Nằm dưới ảnh)
